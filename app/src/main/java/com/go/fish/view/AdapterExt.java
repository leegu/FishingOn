...
import com.go.fish.data.FieldInfoData;
...
public class AdapterExt extends BaseAdapter {
	...
	static final String TAG = AdapterExt.class.getName();
	Object extra;
...
//		}
	...
	private ArrayList<IBaseData> makeArray(JSONArray array){
		...
		case R.layout.listitem_podcast:
			...
			arr = makePodCastDataArray(array);
			break;
	...
	public static ArrayList<IBaseData> makeZixunDataArray(JSONArray array){
		...
		for(int i = 0; i < array.length(); i++) {
			JSONObject jsonObject = array.optJSONObject(i);
			FieldInfoData newsData = FieldInfoData.newInstance(jsonObject);
			arr.add(newsData);
		}
		return arr;
	}
...
//				JSONObject jsonObject = array.optJSONObject(0);
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		case R.layout.listitem_podcast://钓播
			item = onGetPodCastViews(position, convertView, parent);
			break;
		case R.layout.listitem_friend_3_rows:
	...
	private View onGetZixun(int position, View convertView, ViewGroup parent){
		...
		FieldInfoData fPlaceZixunData = (FieldInfoData)data;
...
//			holder.comment_listitem_lower_comments
	...
	private View onGetPodCastViews(final int position, View convertView, ViewGroup parent){
		IBaseData data = listDatas.get(position);
		FNewsViewHolder holder = null;
		View item = null;
		final PodCastData newsData = (PodCastData)data;
		if (convertView == null) {
	...
	private class ScrollListenerImpl implements OnScrollListener {
        ...
        private void loadRecent(){
        	...
        	if(layout_id == R.layout.listitem_podcast){
        		PodCastHelper.getNetPodList(mListView, (String)mListView.getTag(), true);
        	}
        }
        private void loadMore(){
        	LogUtils.d(TAG, "loadMore start =" + listDatas.size());
        	if(layout_id == R.layout.listitem_podcast){
        		PodCastHelper.getNetPodList(mListView, (String)mListView.getTag(), false);
        	}
        }
